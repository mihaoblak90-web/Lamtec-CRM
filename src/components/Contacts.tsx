...
export function Contacts() {
  ...
  const handleAddContact = (e: React.FormEvent) => {
    ...
    setNewContact({ name: '', company: '', role: '', email: '', phone: '', status: 'Lead' }); // reset form
  };

  const getMockOrgChart = (company: string, contactName: string) => {
    return [
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
        ...
        <div className="p-4 border-t border-slate-200 flex items-center justify-between text-sm text-slate-600">
          <div>
            Showing <span className="font-medium">{filteredContacts.length > 0 ? 1 : 0}</span> to <span className="font-medium">{filteredContacts.length}</span> of <span className="font-medium">{contacts.length}</span> results
          </div>
          <div className="flex items-center gap-2">
            <button 
              onClick={() => alert('Previous page')}
              className="p-1.5 rounded-lg border border-slate-300 text-slate-400 hover:text-slate-600 hover:bg-slate-50 disabled:opacity-50"
            >
          ...
          <div className="bg-white rounded-xl shadow-xl w-full max-w-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200 flex flex-col max-h-[90vh]">
            ...
            <div className="flex border-b border-slate-200 shrink-0 overflow-x-auto">
              ...
              <button
                ...
                className={cn(
                  "flex-1 py-3 px-4 text-sm font-medium border-b-2 transition-colors flex items-center justify-center gap-2 whitespace-nowrap",
                  activeTab === 'actions' ? "border-[#0097b2] text-[#0097b2]" : "border-transparent text-slate-500 hover:text-slate-700 hover:border-slate-300"
                )}
              >
              ...
              <button
                ...
                className={cn(
                  "flex-1 py-3 px-4 text-sm font-medium border-b-2 transition-colors flex items-center justify-center gap-2 whitespace-nowrap",
                  activeTab === 'minutes' ? "border-[#0097b2] text-[#0097b2]" : "border-transparent text-slate-500 hover:text-slate-700 hover:border-slate-300"
                )}
              >
                    ...
                    {getMockOrgChart(selectedContact.company, selectedContact.name).map((node, idx) => (
                      <div key={idx} className="relative">
                        ...
                        <div className={cn(
                          ...
                          node.highlight ? "bg-[#0097b2]/5 border-[#0097b2]/20" : "bg-white border-slate-200"
                        )}>
                          <p className={cn("text-sm font-bold", node.highlight ? "text-[#005b7f]" : "text-slate-900")}>
                            {node.name}
                          </p>
                          <p className={cn("text-xs mt-1", node.highlight ? "text-[#0097b2]" : "text-slate-500")}>
                            {node.role}
                          </p>
                        </div>
                      </div>
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <button 
                      ...
                      className="text-sm text-[#0097b2] font-medium hover:text-[#005b7f]"
                    >
                      + Add Action
                    </button>
                  </div>
                  <div className="space-y-3">
                    ...
                    {(!contactActions[selectedContact.id] || contactActions[selectedContact.id].length === 0) && (
                      <div className="text-center py-8 text-slate-500 text-sm">
                        No actions connected to this contact.
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-3">
                    ...
                    {(!contactDocuments[selectedContact.id] || contactDocuments[selectedContact.id].length === 0) && (
                      <div className="text-center py-8 text-slate-500 text-sm">
                        No documents connected to this contact.
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-3">
                    ...
                    {(!contactMinutes[selectedContact.id] || contactMinutes[selectedContact.id].length === 0) && (
                      <div className="text-center py-8 text-slate-500 text-sm">
                        No meeting minutes connected to this contact.
                      </div>
                    )}
                  </div>
                </div>
            ...
            <form onSubmit={handleAddDocument} className="p-4 space-y-4">
              <div>
                ...
                <input 
                  ...
                  className="w-full px-3 py-2 rounded-lg border border-slate-300 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                  placeholder="e.g. Q1 Pricing Agreement"
                />
              </div>
            ...
            <form onSubmit={handleAddMinute} className="p-4 space-y-4">
              ...
              <div>
                ...
                <textarea 
                  ...
                  rows={4}
                  className="w-full px-3 py-2 rounded-lg border border-slate-300 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 resize-none"
            ...
            <form onSubmit={handleAddMinute} className="p-4 space-y-4">
              <div>
                ...
                <input 
                  ...
                  onChange={e => setNewMinute({...newMinute, title: e.target.value})}
                  className="w-full px-3 py-2 rounded-lg border border-slate-300 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                  placeholder="e.g. Q1 Strategy Alignment"
                />
              </div>

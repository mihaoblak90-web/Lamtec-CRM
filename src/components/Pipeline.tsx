...
// Extend mock data with stages for the pipeline
const initialPipelineDeals = initialDealsData.map((deal, index) => ({
  ...deal,
  stage: STAGES[index % STAGES.length].id, // Distribute deals across stages for demo
  id: `deal-${index}`
}));
...
export function Pipeline() {
  ...
  const handleDragStart = (e: React.DragEvent, dealId: string) => {
    ...
    // Make it look nice while dragging
                  ...
                  {stageDeals.map(deal => (
                    ...
                    >
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="flex items-center justify-between text-xs">
                          <span className="text-slate-500 flex items-center gap-1.5">
                            <CalendarIcon className="w-3.5 h-3.5" /> SOP
                          </span>
                          <span className="font-medium text-slate-900">{deal.sop}</span>
                        </div>
                      </div>

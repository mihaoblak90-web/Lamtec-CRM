...
export function CalendarView() {
  ...
  const getLinkedMinutesForVisit = (visitId: string | number) =>
    meetingMinutes.filter((minute) => String(minute.visitId) === String(visitId));
  ...
  const handleAddMinutes = (e: React.FormEvent) => {
    ...
    if (minutesType === 'text' && !minutesContent.trim()) {
      setMinutesError('Please provide notes before saving.');
      return;
    }
    if (minutesType === 'file' && !minutesFile) {
      setMinutesError('Please upload a file before saving.');
      return;
    }
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between p-6 border-b border-slate-200">
              ...
              <button onClick={() => { setIsMinutesModalOpen(false); setSelectedVisit(null); setSelectedLinkedMinute(null); setMinutesError(null); setMinutesContent(''); setMinutesFile(null); }} className="text-slate-400 hover:text-slate-600 transition-colors"><X className="w-5 h-5" /></button>
            </div>
            ...
            <form onSubmit={handleAddMinutes} className="p-6 space-y-6">
              ...
              <div className="space-y-4">
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        if (!allowedExt.includes(ext)) {
                          setMinutesError('Only PDF, DOCX, and TXT files are allowed.');
                          setMinutesFile(null);
                          return;
                        }
                        if (file.size > 10 * 1024 * 1024) {
                          setMinutesError('Maximum file size is 10 MB.');
                          setMinutesFile(null);
                          return;
                        }
                        setMinutesError(null);
                ...
                {minutesError && <p className="text-sm text-rose-600">{minutesError}</p>}
              </div>

              <div className="pt-4 flex justify-end gap-3 border-t border-slate-100">
                <button type="button" onClick={() => { setIsMinutesModalOpen(false); setSelectedVisit(null); setSelectedLinkedMinute(null); setMinutesError(null); setMinutesContent(''); setMinutesFile(null); }} className="px-4 py-2 text-sm font-medium text-slate-700 bg-white border border-slate-300 rounded-lg hover:bg-slate-50">Cancel</button>
